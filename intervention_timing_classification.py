...
import evaluate
import numpy as np
from datasets import Value, load_dataset, load_metric, Dataset
...
class DataTrainingArguments:
    ...
    speakers_info: bool = field(
        default=False,
        metadata={
            "help": "Use additional speaker background information or not."
        },
    )
...
def main():
    ...
    #
    #         processed_dataset.append({"input": input, "output_text": output, "output_role": role_label,
    #                                   "output_moderation": moderator_intervene_label})
    #     df = pd.DataFrame(processed_dataset)
    #     return df

    def preprocess_function(examples):

        examples["sentence"] = [""] * len(examples["debate_id"])
        for i in range(len(examples["debate_id"])):
            input = "Title: " + examples['title'][i] + ".\n"
            if data_args.speakers_info:
                input += "Speakers: " + examples['speakers'][i] + ".\n"
            else:
                input += "Speakers: "
                speaker_names  = ", ".join([s.split(":")[0] for s in examples['speakers'][i].split("\n")])
                input += speaker_names + ".\n"
            dialogue_history = examples['dialogue_history'][i]
            ...
            while(exceed_maxlen):
                skip_utterance += 1
                utterances = dialogue_history.split("\n")
                selected_utterances = utterances[skip_utterance:]
                exceed_maxlen = len(tokenizer.tokenize(input + "Dialogue: " + "\n ".join(selected_utterances) + ".")) > max_seq_length

            input += "Dialogue: " + "\n ".join(selected_utterances) + "."
            examples["sentence"][i] = input
        ...
        if label_to_id is not None and "label" in examples:
            if is_multi_label:
                ...
                if data_args.prediction_task == "intervention_timing":
                    result["label"] = [int((label_to_id[str(l)] == label_to_id["mod"]) if l != -1 else -1) for l in examples["label"]]
                else:
